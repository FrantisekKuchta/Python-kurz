...
for slovo in vycistena_slova:
    if slovo not in shodna_slova:
        shodna_slova[slovo] = 1
    else:
        #shodna_slova[slovo] = shodna_slova[slovo] + 1
        shodna_slova[slovo] += 1

#print(shodna_slova)
nejcasttejsi_hodnoty = sorted(list(shodna_slova.values()), reverse=True)[0:5]
print(nejcasttejsi_hodnoty)
odelovac = "+--+------------+--+"
pet_nejcastejsich = list()

for vyskyt in shodna_slova:
    if shodna_slova[vyskyt] in nejcasttejsi_hodnoty:
        nejcasttejsi_hodnoty.remove(shodna_slova[vyskyt])
        pet_nejcastejsich.append((shodna_slova[vyskyt], vyskyt))

print(pet_nejcastejsich)
    
# sorted()
pet_nejcastejsich = sorted(shodna_slova,key=shodna_slova.get,reverse=True)[0: 5]


#for index, par in enumerate(sorted(pet_nejcastejsich, reverse=True),1):
   # print(odelovac,f"|{index}.|{par[1]: ^10}|{par[0]}x|",odelovac, sep="\n")

for index, nejcasteji in enumerate(pet_nejcastejsich, 1):
    print(odelovac, f"|{index}.|{nejcasteji: ^12}|{shodna_slova[nejcasteji]}x|", odelovac, sep="\n") 
